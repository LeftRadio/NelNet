...
import datetime
import telnetlib
from interface import NDriverInterface


class NTelnetInterface(NDriverInterface):
    ...
    def connect(self, name):
        ...
        if not self.open():
            ...
            if name in self.get_desc_str(0):
                return 0
    ...
    def read(self, rd_buf, nb):
        ...
        self.log( 'read - [%s] %s' % (', '.join([hex(b) for b in rd_buf]), len(rd_buf)) )
        return status

    def write(self, buf, nb):
        """ """
        self.log('write - [%s] %s' % (', '.join([hex(b) for b in buf]), len(buf)) )
...
if __name__ == '__main__':
    ...
    print( nstel.__class__.__name__ )
